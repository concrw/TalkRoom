-- TALKROOM Database - Reset and Migrate
...
DROP TABLE IF EXISTS public.room_favorites CASCADE;
DROP TABLE IF EXISTS public.room_participants CASCADE;
DROP TABLE IF EXISTS public.talk_rooms CASCADE;
DROP TABLE IF EXISTS public.users CASCADE;

-- Drop types
DROP TYPE IF EXISTS public.post_type CASCADE;
DROP TYPE IF EXISTS public.notification_type CASCADE;
DROP TYPE IF EXISTS public.chat_message_type CASCADE;

-- Drop functions
DROP FUNCTION IF EXISTS public.update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS public.sync_feed_post_likes() CASCADE;
DROP FUNCTION IF EXISTS public.get_public_user_profile(uuid) CASCADE;
...
-- 2. TALK_ROOMS TABLE
-- ============================================
CREATE TABLE public.talk_rooms (
...
CREATE POLICY "Public talk rooms are viewable" ON public.talk_rooms
  FOR SELECT USING (is_public = true);

CREATE POLICY "Hosts can create rooms" ON public.talk_rooms
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = host_id);

CREATE POLICY "Hosts can update rooms" ON public.talk_rooms
  FOR UPDATE TO authenticated USING (auth.uid() = host_id);

CREATE POLICY "Hosts can delete rooms" ON public.talk_rooms
  FOR DELETE TO authenticated USING (auth.uid() = host_id);

CREATE INDEX idx_talk_rooms_host_id ON public.talk_rooms (host_id);
CREATE INDEX idx_talk_rooms_starts_at ON public.talk_rooms (starts_at);
...
CREATE POLICY "Users can view participations" ON public.room_participants
  FOR SELECT TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "Users can join rooms" ON public.room_participants
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update participation" ON public.room_participants
...
CREATE POLICY "Users can view favorites" ON public.room_favorites
  FOR SELECT TO authenticated USING (auth.uid() = user_id);
...
CREATE UNIQUE INDEX uq_voice_sessions_active ON public.voice_sessions (talk_room_id) WHERE (is_active);

CREATE POLICY "Anyone can view sessions" ON public.voice_sessions
  FOR SELECT USING (true);

CREATE POLICY "Host can manage sessions" ON public.voice_sessions
...
CREATE POLICY "Users can manage courses" ON public.training_courses
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX idx_training_courses_user ON public.training_courses (user_id);
...
CREATE POLICY "Users can manage logs" ON public.daily_logs
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX idx_daily_logs_user ON public.daily_logs (user_id);
CREATE INDEX idx_daily_logs_room ON public.daily_logs (talk_room_id);
...
CREATE POLICY "Users can manage reviews" ON public.reviews
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX idx_reviews_room ON public.reviews (talk_room_id);
...
CREATE POLICY "Public posts viewable" ON public.feed_posts
  FOR SELECT USING (is_public = true);

CREATE POLICY "Users can manage posts" ON public.feed_posts
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX idx_feed_posts_public ON public.feed_posts (is_public, created_at DESC);
...
CREATE POLICY "Like posts" ON public.post_likes FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Unlike posts" ON public.post_likes FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE TRIGGER trg_talk_rooms_updated BEFORE UPDATE ON public.talk_rooms FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER trg_voice_sessions_updated BEFORE UPDATE ON public.voice_sessions FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER trg_chat_messages_updated BEFORE UPDATE ON public.chat_messages FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER trg_likes_insert AFTER INSERT ON public.post_likes FOR EACH ROW EXECUTE FUNCTION public.sync_feed_post_likes();
CREATE TRIGGER trg_likes_delete AFTER DELETE ON public.post_likes FOR EACH ROW EXECUTE FUNCTION public.sync_feed_post_likes();
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT u.id, u.name, u.rating, u.bio, u.avatar_url FROM public.users u WHERE u.id = _id;
$$;
